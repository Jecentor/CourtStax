...
function loadSessions() {
  try {
    ...
    const sess = loaded.sessions || {};
    // backfill fields added after some sessions were already saved
...
// ---------- lightweight error logging (no paid service needed) ----------
const ERROR_LOG_FILE = path.join(DATA_DIR, "error.log");
function logError(code, actionName, err) {
  const line = `[${new Date().toISOString()}] session=${code || "-"} action=${actionName || "-"} error=${err.message}\n`;
  fs.appendFile(ERROR_LOG_FILE, line, () => {}); // fire-and-forget, never block the response
...
function makePlayer(name, skill) {
  return {
    ...
    duprId: "",
    duprConnected: false,
    totalPlayMs: 0, // cumulative time spent actually playing, summed across all their games
  };
}
...
function syncCourtsCount(s, count, force) {
  const willDrop = s.courts.slice(count).some((c) => c.players.length > 0);
  if (willDrop && !force) {
    throw new Error("CONFIRM_DROP_ACTIVE_COURT"); // caught by the client to show a confirm modal
  }
  const next = [...s.courts];
...
const actions = {
  finishSession(s, { finished }) {
    s.finished = !!finished;
    s.finishedAt = s.finished ? Date.now() : null;
  },

  checkin(s, { name, skill }) {
    if (s.finished) throw new Error("This session has ended. Reopen it first to check in more players.");
    if (!name || !name.trim()) throw new Error("Name required");
    const cleanName = name.trim().slice(0, 40); // cap length — protects UI layout + storage
    let skillNum = parseFloat(skill);
    if (!Number.isFinite(skillNum)) skillNum = 3.0;
    skillNum = Math.min(7.0, Math.max(1.0, skillNum)); // clamp — bad values wreck matchup scoring
    const p = makePlayer(cleanName, skillNum);
    s.players[p.id] = p;
    s.queue.push(p.id);
  },
  ...
  setStatus(s, { id, status }) {
    ...
    const validStatuses = ["ready", "resting", "leaving", "injured"];
  ...
  removePlayer(s, { id }) {
    const p = s.players[id];
    if (!p) throw new Error("Player not found");
    // snapshot for undo — organizers misclick "remove" during live sessions constantly
    s._undo = {
      type: "removePlayer",
      player: p,
      queuePos: s.queue.indexOf(id),
      courtId: (s.courts.find((c) => c.players.includes(id)) || {}).id || null,
  ...
  undoLast(s) {
    const u = s._undo;
    if (!u || Date.now() > u.expiresAt) throw new Error("Nothing to undo (or it expired).");
    if (u.type === "removePlayer") {
      s.players[u.player.id] = u.player;
      if (u.courtId) {
        const court = s.courts.find((c) => c.id === u.courtId);
        if (court && court.players.length < 4) court.players.push(u.player.id);
        else s.queue.splice(Math.max(0, u.queuePos), 0, u.player.id);
      } else {
        s.queue.splice(Math.max(0, u.queuePos), 0, u.player.id);
      }
    } else if (u.type === "recordWinner") {
      const court = s.courts.find((c) => c.id === u.courtId);
      if (court) {
        court.players = u.courtPlayers;
        court.startedAt = u.startedAt;
      }
      u.playerIds.forEach((id) => {
        s.queue = s.queue.filter((pid) => pid !== id);
      });
      s.players = { ...s.players, ...u.playerSnapshots };
      s.round = u.roundBefore;
    }
    s._undo = null;
  },
  ...
  setDuprSettings(s, { clubId, apiKey }) {
    ...
    // Only overwrite the key if a new one was actually typed. The client never
    // receives the real key back (see toClientSession), so a blank field on
    // save must mean "unchanged", not "delete my key".
  ...
  setCourtsCount(s, { count, force }) {
    syncCourtsCount(s, Math.max(1, Math.min(12, parseInt(count, 10) || 1)), !!force);
  },
  ...
  fillCourt(s, { courtId }) {
    if (s.finished) throw new Error("This session has ended. Reopen it first to start more games.");
  ...
  recordWinner(s, { courtId, side }) {
    if (s.finished) throw new Error("This session has ended. Reopen it first to log more results.");
    const court = s.courts.find((c) => c.id === courtId);
    if (!court || court.players.length < 4) throw new Error("Court is not full");
    const [p1, p2, p3, p4] = court.players;
    const elapsedMs = court.startedAt ? Math.max(0, Date.now() - court.startedAt) : 0;
    ...
    s._undo = {
      ...
      playerSnapshots: Object.fromEntries(
        court.players.filter((id) => s.players[id]).map((id) => [id, { ...s.players[id] }])
      ),
      roundBefore: s.round,
      expiresAt: Date.now() + 30000,
    };
    ...
    court.players.forEach((id) => {
      ...
      p.gamesPlayed += 1;
      p.totalPlayMs = (p.totalPlayMs || 0) + elapsedMs;
...
// ---------- client-safe session view ----------
// NEVER send a raw session object to a client — it contains the DUPR API key
// in plaintext. This strips/masks anything secret before a response goes out.
function toClientSession(s) {
  const { apiKey, ...restDuprSettings } = s.duprSettings;
  return {
    ...s,
    duprSettings: {
      ...restDuprSettings,
      apiKeyMasked: apiKey ? "•".repeat(Math.max(0, apiKey.length - 4)) + apiKey.slice(-4) : "",
    },
  };
}

// ---------- stale session cleanup ----------
// Beta sessions pile up forever otherwise — one JSON file with hundreds of
// dead test clubs in it. Sweep anything untouched for 48h+.
...
function sweepStaleSessions() {
  ...
  for (const code of Object.keys(sessions)) {
    ...
    const lastActive = s.lastActivityAt || s.createdAt || 0;
  ...
  if (removed > 0) {
    console.log(`Swept ${removed} stale session(s) inactive 48h+`);
    persist();
  }
}
setInterval(sweepStaleSessions, 60 * 60 * 1000); // hourly
sweepStaleSessions(); // also run once on boot
...
const MAX_BODY_BYTES = 100 * 1024; // 100KB — generous for this app's payloads, blocks abuse
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("data", (chunk) => {
      ...
      if (bytes > MAX_BODY_BYTES) {
        tooLarge = true;
        return; // keep draining the socket so the connection can close cleanly, just stop storing it
      }
      body += chunk;
    });
    req.on("end", () => {
...
const server = http.createServer(async (req, res) => {
  ...
  if (url === "/api/session" && req.method === "POST") {
    try {
      ...
      persist();
      return send(res, 200, { code, ...toClientSession(sessions[code]) });
    } catch (e) {
  ...
  if (m && req.method === "GET") {
    ...
    return send(res, 200, { code, ...toClientSession(s) });
  }
...
process.on("uncaughtException", (err) => {
  logError(null, "uncaughtException", err);
});
process.on("unhandledRejection", (err) => {
  logError(null, "unhandledRejection", err instanceof Error ? err : new Error(String(err)));
});
